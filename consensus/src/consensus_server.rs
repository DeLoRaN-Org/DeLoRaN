...
use tonic::{transport::{Certificate, Identity, ServerTlsConfig}, Request, Response};

use crate::{consensus_client::ConsensusClient, get_addr, protos::{uplink_deduplication_consensus_server::{UplinkDeduplicationConsensus, UplinkDeduplicationConsensusServer}, ReceptionSetDisseminationRequest, ReceptionSetDisseminationResponse, UplinkReceivedDisseminationRequest, UplinkReceivedDisseminationResponse}, ConsensusCerts, ConsensusError, ConsensusMessage, ConsensusRound, ConsensusState};
...
impl ConsensusServer {
    fn new(id: String, receiver: Receiver<ConsensusMessage>, certs: ConsensusCerts) -> Self {
        ...
        tokio::spawn(ConsensusServer::consensus_receiver_broadcaster_routine(cloned_rounds, receiver, id.clone(), certs.clone()));
        ConsensusServer {
            id,
            rounds,
        }
    ...
    pub fn run_instance(id: String, config: ConsensusConfig) -> Result<Sender<ConsensusMessage>, ConsensusError> {
        ...
        let (sender, receiver) = tokio::sync::mpsc::channel::<ConsensusMessage>(100);
        
        tokio::spawn(tonic::transport::Server::builder().tls_config(server_tls).map_err(|_| ConsensusError::InvalidTlsConfig)?
        .timeout(Duration::from_millis(300))
        .add_service(UplinkDeduplicationConsensusServer::new(ConsensusServer::new(id, receiver, config.certs))).serve(config.addr));

        Ok(sender)
    }

    async fn consensus_receiver_broadcaster_routine(rounds: Rounds, mut receiver: Receiver<ConsensusMessage>, id: String, certs: ConsensusCerts) {
        while let Some(msg) = receiver.recv().await {
            let mut nc_set = HashMap::new();
            nc_set.insert(id.clone(), msg.rssi);
            let dev_addr: String = msg.dev_addr.clone();

            rounds.write().await.insert(dev_addr, Mutex::new(ConsensusRound {
                started_at: Instant::now(),
                status: ConsensusState::ReceivingDisseminations,
                nc_list: msg.nc_list.clone(),
                nc_set,
                packet: msg.packet.clone(),
                validated_sets: Vec::from([id.clone()]),
                sender: msg.response,
            }));
            ...
            for nc in list.iter() {
                if nc != &id {
                    ...
                    let mut client = ConsensusClient::new(format!("https://{}", get_addr(nc)), certs.clone()).await.unwrap();
                    match client.broadcast_reception(&msg.dev_addr, &msg.packet, msg.rssi).await {
                        Ok(UplinkReceivedDisseminationResponse { answer }) => {
                            if let Some(answer) = answer {
                                if let Err(_e) = ConsensusServer::broadcast_reception_handler(&id, nc, &rounds, answer).await {
                                    //eprintln!("[{id}]: {e:?}");
                                }
                            }
                        },
    ...
    pub async fn add_nc_to_set(rounds: &Rounds, dev_addr: &str, nc_id: &str, rssi: i32) -> bool {
        if let Some(round) = rounds.read().await.get(dev_addr) {
            ...
            if !round.nc_list.contains(&nc_id) {
                ...
                round.nc_set.insert(nc_id.clone(), rssi);
    ...
    pub async fn end_round(rounds: &Rounds, dev_addr: &str) -> Option<ConsensusRound> {
        rounds.write().await.remove(dev_addr).map(|round| {
            round.into_inner()
        })
    }
    ...
    pub fn extract_cn_from_certificate(certificates: Option<std::sync::Arc<Vec<Certificate>>>) -> Option<String> {
        if let Some(certs) = certificates {
            if certs.is_empty() {
                ...
                pem.subject_name().entries().find(|name| name.object().nid() == Nid::COMMONNAME)
    ...
    pub fn create_dissemination_request(dev_addr: &str, packet: &[u8], rssi: i32) -> UplinkReceivedDisseminationRequest {
        let mut hasher = Sha256::new();
    ...
    pub async fn check_dissemination_set_request(name: String, r: &ReceptionSetDisseminationRequest, round: &mut ConsensusRound) -> Result<bool, ConsensusError> {
        ...
        if round.validated_sets.contains(&name) {
            return Err(ConsensusError::NCAlreadyInSet)
        ...
        if r.set == round.nc_set {
            round.validated_sets.push(name);
        }
        if round.validated_sets.len() == round.nc_list.len() {
    ...
    pub async fn broadcast_reception_handler(id: &str, src: &str, rounds: &Rounds, r: UplinkReceivedDisseminationRequest) -> Result<Response<UplinkReceivedDisseminationResponse>, tonic::Status> {
        ...
        match ConsensusServer::check_round(rounds, &r, src).await {
            Err(e) => {
                if e == ConsensusError::NCAlreadyInSet {
                    Ok(Response::new(UplinkReceivedDisseminationResponse { answer: None }))
                } else {
                    Err(tonic::Status::aborted(format!("{:?}", e)))
                }
            }
            Ok(_) => {
                let added = ConsensusServer::add_nc_to_set(rounds, &r.dev_addr, src, r.rssi).await;
                if !added {
                    return Err(tonic::Status::unauthenticated("Not part of the consensus round"))
                }
                ...
                let dissemination_request = {
                    ...
                    ConsensusServer::create_dissemination_request(&r.dev_addr, packet, *inner_round.nc_set.get(id).expect("NC id should always be in the set"))
                };
                ...
                Ok(Response::new(UplinkReceivedDisseminationResponse { answer: Some(dissemination_request)}))
            }
        }
    }

    pub async fn broadcast_nc_set_handler(id: &str, src: String, rounds: &Rounds, r: ReceptionSetDisseminationRequest) -> ConsensusResult<ReceptionSetDisseminationResponse> {
        ...
        match result {
            Ok((nc_set, ended, winner)) => {
                if ended && winner {
                    let rounds = rounds.read().await;
                    let round = rounds.get(&r.dev_addr).unwrap().lock().await;
                    let duration = round.started_at.elapsed();
                    println!("### {id} WON! after {duration:?} ###");
                    } else if ended && !winner {
                    let rounds = rounds.read().await;
                    let round = rounds.get(&r.dev_addr).unwrap().lock().await;
                    let duration = round.started_at.elapsed();
                    println!("*** {id} NOT WON  after {duration:?} ***")
                }
        
                if ended {
                    if let Some(round) = ConsensusServer::end_round(rounds, &r.dev_addr).await {
                        round.sender.send(winner).unwrap();
                    }
                }
                Ok(Response::new(ReceptionSetDisseminationResponse { answer: Some(ReceptionSetDisseminationRequest { 
                    dev_addr: r.dev_addr, 
                    set: nc_set 
                })}))
            },
            Err(e) => {
                if e == ConsensusError::NCAlreadyInSet {
                    Ok(Response::new(ReceptionSetDisseminationResponse { answer: None }))
                } else {
                    Err(tonic::Status::aborted(format!("{:?}", e)))
                }
            }
...
impl UplinkDeduplicationConsensus for ConsensusServer {
    async fn broadcast_reception(&self, request: Request<UplinkReceivedDisseminationRequest>) -> ConsensusResult<UplinkReceivedDisseminationResponse> {
        ...
        if let Some(name) = name {
            ...
            ConsensusServer::broadcast_reception_handler(&self.id, &name, rounds, request.into_inner()).await
